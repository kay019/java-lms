...
public class JdbcSessionRepository implements SessionRepository {
    ...
    public int save(Session session) {
        String sql = "insert into session (course_id, title, session_type, max_enrollment, fee, start_date, finish_date, status, enrollmentCount) values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
        return jdbcTemplate.update(sql, session.getCourseId(), session.getTitle(),
                ...
                session.getStatus(), session.getEnrollmentCount());
    }

    @Override
    public Session findById(Long id) {
        String sql = "select id, course_id, title, session_type, max_enrollment, fee, start_date, finish_date, status, enrollmentCount from session where id = ?";
